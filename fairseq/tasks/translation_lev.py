...
class TranslationLevenshteinTask(TranslationTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            choices=['random_delete', 'random_mask', 'no_noise', 'full_mask', 'span_mask'])
        # parser.add_argument(
    ...
    def inject_noise(self, target_tokens):
        ...
        def _span_mask(target_tokens):
            ...
            target_lengths = target_masks.sum(1).float()
            B, T = target_tokens.size()

            prev_target_tokens = []
  
            for b in range(B):
                length = int(target_lengths[b])

                start = torch.randint(1, length+1, (1,))
                end = torch.randint(int(start), length+1, (1,))
                # print(start, end)
                _tokens = target_tokens[b].int().tolist()
                _prev_target = _tokens[:start] + [unk]*(end-start) + _tokens[end:]
                prev_target_tokens.append(torch.IntTensor(_prev_target).to(target_tokens.device))
            prev_target_tokens = torch.stack(prev_target_tokens,dim=0)
            # import pdb
            # pdb.set_trace()
            # print(prev_target_tokens)
            # xxx
            return prev_target_tokens
            # return
        ...
        elif self.args.noise == "span_mask":
            return _span_mask(target_tokens)
        else:
