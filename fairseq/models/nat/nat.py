...
def _fast_multinomial(exp,topk=1):
    rand = torch.empty_like(exp).uniform_()
    indices = (rand.log()/exp).topk(k=topk,dim=-1).indices
    return indices.to(exp.device)
...
class NATransformerModel(FairseqNATModel):
    ...
    def sample(
        self, src_tokens, src_lengths, prev_output_tokens, tgt_tokens, n_sample=1, normalize=True, train_ratio=None, with_forward=False, temperature=1, glat=None, **kwargs
    ):
        outputs = {}
        ...
        # prev_output_tokens = torch.full((B*S,max_leng),self.pad)
        prev_output_tokens = torch.zeros([B*S,max_leng]).long().cuda()
        ...
        idx = torch.cumsum((prev_output_tokens==2),1)
        sent_idx = idx < 1
        pad_idx = ~sent_idx

        # assign tokens according to idx
        prev_output_tokens[sent_idx] = self.unk
    ...
    def initialize_output_tokens(self, encoder_out, src_tokens):
        # length prediction
        length_tgt, log_legnth_prob = self.decoder.forward_length_prediction(
            self.decoder.forward_length(normalize=True, encoder_out=encoder_out),
            encoder_out=encoder_out,
        )
        length_tgt = length_tgt.squeeze(-1)
...
class NATransformerDecoder(FairseqNATSharedDecoder):
    ...
    def forward_length_prediction(self, length_out, encoder_out, tgt_tokens=None, num_samples=0, beam_around=False):
        enc_feats = encoder_out["encoder_out"][0]  # T x B x C
        if len(encoder_out["encoder_padding_mask"]) > 0:
            src_masks = encoder_out["encoder_padding_mask"][0]  # B x T
        else:
        ...
        if tgt_tokens is not None:
            if num_samples > 0 :
                ...
                if self.pred_length_offset:
                    golden_length_tgt = tgt_lengs - src_lengs + 128

                else:
                    golden_length_tgt = tgt_lengs
                golden_length_tgt = golden_length_tgt.clamp(min=0, max=255)
                
                
                # sampling lengths
                length_prob = length_out.exp()
                pred_lengs = torch.multinomial(length_prob, num_samples=num_samples, replacement=True)
                
                log_length_prob = torch.gather(length_prob, 1, pred_lengs).log()
                if self.pred_length_offset:
                    sampled_length_tgt = pred_lengs - 128 + src_lengs.unsqueeze(-1)

                else:
                    sampled_length_tgt = pred_lengs
                
                sampled_length_tgt = sampled_length_tgt.clamp(min=2, max=255)
                # clamp out lenghth with too much deviation
                min_tgt_lengs = torch.min(tgt_lengs)
                max_tgt_lengs = torch.max(tgt_lengs)
                sampled_length_tgt = sampled_length_tgt.clamp(min=min_tgt_lengs-self.max_length_bias, max=max_tgt_lengs+self.max_length_bias)

                return sampled_length_tgt, log_length_prob, golden_length_tgt
            ...
            if not beam_around:
                ...
                length_prob = length_out.exp()
                length_prob, pred_lengs = torch.topk(length_prob, max(num_samples,1), dim=-1,)
                log_length_prob = length_prob.log()
                if self.pred_length_offset:
                    length_tgt = pred_lengs - 128 + src_lengs.unsqueeze(-1)
                else:
                    length_tgt = pred_lengs
                length_tgt = length_tgt.clamp(min=2, max=255)
                # print(length_tgt)
                
                # best_leng = length_tgt[0]
                # # print(pred_lengs)
                # length_tgt = length_tgt.clamp(min=best_leng-(self.max_length_bias//2+1), max=best_leng+(self.max_length_bias//2+1))
                # print(length_tgt)
                # xxx
                return length_tgt, log_length_prob
                # else:
                ...
                length_prob = length_out.exp()
                _, pred_lengs = torch.topk(length_prob, 1, dim=-1,)
                log_length_prob = length_prob.log()

                pred_lengs = (
                            pred_lengs
                            + utils.new_arange(pred_lengs, 1, num_samples)
                            - num_samples // 2
                        )   
          
                log_length_prob = torch.gather(log_length_prob, 1, pred_lengs)
                if self.pred_length_offset:
                    length_tgt = pred_lengs - 128 + src_lengs.unsqueeze(-1)
                else:
                    length_tgt = pred_lengs
