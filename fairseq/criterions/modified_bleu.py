...
def sentence_gleu_mod(references, hypothesis, min_len=1, max_len=4):
    """
...
def corpus_gleu(list_of_references, hypotheses, min_len=1, max_len=4):
    ...
    for references, hypothesis in zip(list_of_references, hypotheses):
        ...
        if hyp_counts:
            n_match, n_all = max(hyp_counts, key=lambda hc: hc[0] / sum(hc[1]))
            corpus_n_match += n_match
            corpus_tpfp += n_all[0]
            corpus_tpfn += n_all[1]

    # corner case: empty corpus or empty references---don't divide by zero!
    if corpus_tpfp*corpus_tpfn == 0:
        gleu_score = 0.0
    else:
        p = corpus_n_match / corpus_tpfp
        r = corpus_n_match / corpus_tpfn
        # print(corpus_n_match)
        # print(corpus_tpfp)
        # print(corpus_tpfn)
        # print(p,r)
        f = 10*p*r / (r+9*p) if r*p != 0 else 0.
        gleu_score = f
